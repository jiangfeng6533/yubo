...
Page({
      ...
      if (res.data.code == 200) {
        ...
        for (var key in area) {
          if (startarr[area[key].second]) {
            startarr[area[key].second].push({ start: area[key].user_name, second: area[key].second, id: area[key].id, phone: area[key].phone, com_name: area[key].com_name, user_name: area[key].user_name});
          } else {
            startarr[area[key].second] = [{ start: area[key].user_name, second: area[key].second, id: area[key].id, phone: area[key].phone, com_name: area[key].com_name, user_name: area[key].user_name}];
          }
        }
  ...
  changeClient(e) {
    ...
    beforePage.resclientinfo(audit);
  },
  // input监听搜索
  getclientsearch(e) {
    var that = this;
